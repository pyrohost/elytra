...
func newDiagnosticsCommand() *cobra.Command {
	command := &cobra.Command{
		Use:   "diagnostics",
		Short: "Collect and report information about this Elytra instance to assist in debugging.",
...
// diagnosticsCmdRun collects diagnostics about elytra, its configuration and the node.
// We collect:
// - elytra and docker versions
...
func diagnosticsCmdRun(*cobra.Command, []string) {
	...
	fmt.Fprintln(output, "Pterodactyl Elytra - Diagnostics Report")
	printHeader(output, "Versions")
	fmt.Fprintln(output, "               Elytra:", system.Version)
	if dockerErr == nil {
	...
	printHeader(output, "Elytra Configuration")
	...
	printHeader(output, "Latest Elytra Logs")
	if diagnosticsArgs.IncludeLogs {
		p := "/var/log/pterodactyl/elytra.log"
		if cfg != nil {
			p = path.Join(cfg.System.LogDirectory, "elytra.log")
		}
