...
//
// This handles Elytra exiting during either of these processes which will leave
// things in a bad state within the Panel. This API call is executed once Elytra
...
func (c *client) GetServerRusticConfig(ctx context.Context, serverUuid string, backupType string) (RusticBackupRequest, error) {
	var data RusticBackupRequest
	res, err := c.Get(ctx, fmt.Sprintf("/servers/%s/rustic-config", serverUuid), q{"type": backupType})
	if err != nil {
		return data, err
	}
	defer res.Body.Close()
