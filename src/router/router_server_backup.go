...
func postServerBackup(c *gin.Context) {
	...
	case backup.RusticLocalBackupAdapter:
		adapter = backup.NewRusticLocal(client, data.Uuid, data.Ignore)
	case backup.RusticS3BackupAdapter:
		adapter = backup.NewRusticS3(client, data.Uuid, data.Ignore)
	default:
...
func postServerRestoreBackup(c *gin.Context) {
	...
	var data struct {
		Adapter           backup.AdapterType `binding:"required,oneof=elytra s3 rustic-local rustic-s3" json:"adapter"`
	...
	case backup.RusticLocalBackupAdapter:
		...
		go func(s *server.Server, b backup.BackupInterface, logger *log.Entry) {
			logger.Info("starting restoration process for server backup using rustic local driver")
			if err := s.RestoreBackup(b, nil); err != nil {
				logger.WithField("error", err).Error("failed to restore rustic local backup to server")
			}
			s.Events().Publish(server.DaemonMessageEvent, "Completed server restoration from rustic local backup.")
			s.Events().Publish(server.BackupRestoreCompletedEvent, "")
			logger.Info("completed server restoration from rustic local backup")
			s.SetRestoring(false)
		}(s, b, logger)
		hasError = false
		c.Status(http.StatusAccepted)
	...
	case backup.RusticS3BackupAdapter:
		...
		go func(s *server.Server, b backup.BackupInterface, logger *log.Entry) {
			logger.Info("starting restoration process for server backup using rustic S3 driver")
			if err := s.RestoreBackup(b, nil); err != nil {
				logger.WithField("error", err).Error("failed to restore rustic S3 backup to server")
			}
			s.Events().Publish(server.DaemonMessageEvent, "Completed server restoration from rustic S3 backup.")
			s.Events().Publish(server.BackupRestoreCompletedEvent, "")
			logger.Info("completed server restoration from rustic S3 backup")
			s.SetRestoring(false)
		}(s, b, logger)
		hasError = false
		c.Status(http.StatusAccepted)
...
// deleteServerBackup deletes a backup of a server. If the backup is not
