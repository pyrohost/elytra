...
// The time at which Elytra was booted. No JWT's created before this time are allowed to
// connect to the socket since they may have been marked as denied already and therefore
// could be invalid at this point.
//
// By doing this we make it so that a user who gets disconnected from Elytra due to a Elytra
// reboot just needs to request a new token as if their old token had expired naturally.
var elytraBootTime = time.Now()
