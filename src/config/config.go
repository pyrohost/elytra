...
type Backups struct {
	...
	// Rustic configuration for encrypted, deduplicated backups
	Rustic RusticConfiguration `yaml:"rustic"`
}

// RusticConfiguration defines the configuration for rustic-powered backups
type RusticConfiguration struct {
	// Enabled determines if rustic backups are available on this instance
	Enabled bool `default:"true" yaml:"enabled"`

	// Password for the rustic repository. This should be a strong, unique password.
	// Can be set via environment variable RUSTIC_PASSWORD or password file.
	Password string `yaml:"password"`

	// PasswordFile path to a file containing the rustic repository password
	PasswordFile string `yaml:"password_file"`

	// CacheDir specifies the directory to use for rustic cache
	CacheDir string `default:"/var/cache/elytra/rustic" yaml:"cache_dir"`

	// LogLevel for rustic operations (error, warn, info, debug, trace)
	LogLevel string `default:"info" yaml:"log_level"`

	// CompressionLevel for rustic backups (0-22, higher = better compression)
	CompressionLevel int `default:"3" yaml:"compression_level"`

	// Local repository configuration
	Local RusticLocalConfiguration `yaml:"local"`

	// S3 repository configuration
	S3 RusticS3Configuration `yaml:"s3"`
}

// RusticLocalConfiguration defines settings for local rustic repositories
type RusticLocalConfiguration struct {
	// Path where local rustic repositories will be stored
	Path string `default:"/var/lib/pterodactyl/backups/rustic" yaml:"path"`

	// RetentionPolicy defines how long to keep backups
	RetentionPolicy RusticRetentionPolicy `yaml:"retention"`
}

// RusticS3Configuration defines settings for S3 rustic repositories
type RusticS3Configuration struct {
	// Endpoint for S3-compatible storage (leave empty for AWS S3)
	Endpoint string `yaml:"endpoint"`

	// Bucket name for storing backups
	...
	Prefix string `default:"elytra-backups" yaml:"prefix"`

	// RetentionPolicy defines how long to keep backups
	RetentionPolicy RusticRetentionPolicy `yaml:"retention"`
}

// RusticRetentionPolicy defines backup retention settings
type RusticRetentionPolicy struct {
	// KeepDaily number of daily backups to keep
	KeepDaily int `default:"7" yaml:"keep_daily"`

	// KeepWeekly number of weekly backups to keep
	KeepWeekly int `default:"4" yaml:"keep_weekly"`

	// KeepMonthly number of monthly backups to keep
	KeepMonthly int `default:"6" yaml:"keep_monthly"`

	// KeepYearly number of yearly backups to keep
	KeepYearly int `default:"2" yaml:"keep_yearly"`

	// PruneAfterForget whether to prune the repository after forgetting snapshots
	PruneAfterForget bool `default:"true" yaml:"prune_after_forget"`
}
...
func ConfigureDirectories() error {
	...
	// Create rustic directories if rustic is enabled
	if _config.System.Backups.Rustic.Enabled {
		log.WithField("path", _config.System.Backups.Rustic.Local.Path).Debug("ensuring rustic local repository directory exists")
		if err := os.MkdirAll(_config.System.Backups.Rustic.Local.Path, 0o700); err != nil {
			return err
		}

		log.WithField("path", _config.System.Backups.Rustic.CacheDir).Debug("ensuring rustic cache directory exists")
		if err := os.MkdirAll(_config.System.Backups.Rustic.CacheDir, 0o700); err != nil {
			return err
		}
	}
